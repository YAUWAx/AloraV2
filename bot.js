        ...
        if (config.WORKTYPE == 'public') {
            if (config.LANG == 'SI' || config.LANG == 'EN') {
                ...
                if (conn.user.jid === '@s.whatsapp.net') {
                    ...
                    await conn.sendMessage(conn.user.jid, '⛄ *AloraV2 public ආකාරයට ක්‍රියාත්මක විය.👸*\n\n_කරුණාකර මෙහි command උත්සාහ නොකරන්න. මෙය ඔබගේ ලොග් අංකයයි._\n_ඔබට ඕනෑම චැට් එකක විධාන උත්සාහ කළ හැකිය :)_\n\n*ඔබේ command list එක ලබාගැනීමට .alora command එක භාවිතා කරන්න.*\n\n*ඔබේ bot public ක්‍රියාත්මක වේ. වෙනස් කිරීමට* _.setvar WORK_TYPE:private_ *විධානය භාවිතා කරන්න.*\n\n*github - https://github.com/YAUWAx/Alorav2*\n\n*AloraV2 භාවිතා කිරීම ගැන ස්තූතියි ❤👸*', MessageType.text);
                }
            }
            else {
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    if (sendMsg) {
                        ...
                        try {
                            ...
                            if (config.LANG == 'SI' || config.LANG == 'EN') {
                                await conn.sendMessage(conn.user.jid, '*📵 දෝෂ වාර්තාව [ALORA-V2] 📵*' + 
                                    ...
                                    '\n\n_https://chat.whatsapp.com/GV6txkbt3D6HKoNexxkzOF ඔබට එය මෙම group යොමු කළ හැකිය._\n\n' +
                                    '*සිදු වූ දෝෂය:* ```' + error + '```\n\n'
                                    , MessageType.text, {detectLinks: false});
                            } else {
                                await conn.sendMessage(conn.user.jid, '*📵 බොට් වාර්තාව [ALORA-V2] 📵*' + 
                                    ...
                                    '\n\n_ඔබේ bot සඳහා යම් උදව්වක් අවශ්‍ය නම්, https://chat.whatsapp.com/GV6txkbt3D6HKoNexxkzOF_\n\n' +
                                    '*සිදු වූ දෝෂය:* ```' + error + '```\n\n'
                                    , MessageType.text);
                          
                                await conn.sendMessage(conn.user.jid, '*👸💃Queen Alora announcement💃👸*' + 
